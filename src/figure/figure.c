...
static void figure_load(buffer *buf, figure *f, int figure_buf_size)
{
    ...
    // The following code is executed if the savegame includes more figure information than what is supported by
    // the original game.
    if (figure_buf_size > FIGURE_ORIGINAL_BUFFER_SIZE) {
